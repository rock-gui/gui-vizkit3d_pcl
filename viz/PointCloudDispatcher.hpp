...
namespace vizkit3d
{
    ...
    public:
        ...
        static void dispatch(const pcl::PointCloud<pcl::PointXYZ>& pc, osg::ref_ptr<osg::Vec3Array> osg_points,
                             osg::ref_ptr<osg::Vec4Array> osg_colors, const osg::Vec4f& default_feature_color,
                             bool show_color, bool show_intensity, bool useHeightColoring, double maxz, int downsampleSkip)
        {
            ...
            for(size_t i = 0; i < pc.size(); i += downsampleSkip)
            {
                             ...
                             bool show_color, bool show_intensity, bool useHeightColoring, double maxz, int downsampleSkip)
        {
            ...
            else if(show_intensity && pcl::getFieldIndex(pcl_cloud, "intensity") != -1)
            {
                ...
                pcl::fromPCLPointCloud2(pcl_cloud, pc);
                PointCloudDispatcher::dispatch(pc, osg_points, osg_colors, default_feature_color, show_color, show_intensity, useHeightColoring, maxz, downsampleSkip);
            }
            else
            {
                pcl::PointCloud<pcl::PointXYZ> pc;
